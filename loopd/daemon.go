...
func (d *Daemon) initialize(withMacaroonService bool) error {
	...
	if d.cfg.EnableExperimental {
		...
		if d.assetClient != nil {
			...
			assetDepositManager = assets_deposit.NewDepositManager(
				assetDepositClient, d.lnd.WalletKit,
				d.lnd.ChainNotifier, d.assetClient,
				depositStore,
			)
		}
	}
	...
	if assetDepositManager != nil {
		...
		go func() {
			...
			log.Info("Starting asset deposit manager...")
			...
			log.Info("Asset deposit manager stopped")
		}()
		// TODO(bhandras)
