...
func (t *TapdClient) PrepareAndCommitVirtualPsbts(ctx context.Context,
	...
	sponsoringInputs []*taprpc.OutPoint, pinputs []psbt.PInput) (
	...
	*assetwalletrpc.CommitVirtualPsbtsResponse, error) {
	...
	if sponsoringInputs != nil {
		for i, utxo := range sponsoringInputs {
			op := wire.OutPoint{
			...
			btcPkt.Inputs = append(btcPkt.Inputs, pinputs[i])
		}
	}
