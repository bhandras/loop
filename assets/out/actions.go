...
func (o *OutFSM) InitSwapOut(initCtx fsm.EventContext) fsm.EventType {
		...
		for {
			...
			case result := <-trackChan:
				...
				if result.State == lnrpc.Payment_FAILED {
					...
					err = o.SendEvent(fsm.OnError, nil)
					if err != nil {
				...
				if result.State == lnrpc.Payment_SUCCEEDED {
					...
					err := o.SendEvent(onPrepaySettled, nil)
					if err != nil {
						o.Errorf("unable to send event: %v", err)
					}
					return
...
func (o *OutFSM) FetchProof(_ fsm.EventContext) fsm.EventType {
...
func (o *OutFSM) waitForBlock(_ fsm.EventContext) fsm.EventType {
	blockHeight := o.cfg.BlockHeightSubscriber.GetBlockHeight()

	cb := func() {
		o.SendEvent(onBlockReceived, nil)
	}
...
func (o *OutFSM) subscribeToHtlcTxConfirmed(
	_ fsm.EventContext) fsm.EventType {
	...
	confCallback := func(conf *chainntnfs.TxConfirmation, err error) {
		if err != nil {
			...
			o.SendEvent(fsm.OnError, nil)
		}
		cancel()
		o.SendEvent(onHtlcTxConfirmed, conf)
	}
...
func (o *OutFSM) sendSwapPayment(event fsm.EventContext) fsm.EventType {
		...
		for {
			...
			case result := <-trackChan:
				...
				if result.State == lnrpc.Payment_SUCCEEDED {
					...
					err := o.SendEvent(onSwapPreimageReceived, nil)
					if err != nil {
						o.Errorf("unable to send event: %v", err)
					}
					return
...
// confirmation.
func (o *OutFSM) subscribeSweepConf(
	_ fsm.EventContext) fsm.EventType {
