...
type Querier interface {
	...
	SetAssetDepositTimeoutSweepAddr(ctx context.Context,
		arg sqlc.SetAssetDepositTimeoutSweepAddrParams) error
}
...
func (s *SQLStore) MarkDepositExpired(ctx context.Context, depositID string,
	timeoutSweepAddr string) error {

	txOptions := loopdb.NewSqlWriteOpts()

	return s.db.ExecTx(ctx, txOptions, func(tx Querier) error {
		err := tx.SetAssetDepositTimeoutSweepAddr(
			ctx, sqlc.SetAssetDepositTimeoutSweepAddrParams{
				DepositID: depositID,
				TimeoutSweepAddr: sql.NullString{
					String: timeoutSweepAddr,
					Valid:  true,
				},
			},
		)
		...
		return tx.UpdateDepositState(ctx, sqlc.UpdateDepositStateParams{
			DepositID:       depositID,
			UpdateState:     int32(StateExpired),
			UpdateTimestamp: s.clock.Now().UTC(),
		})
	})
}
...
func (s *SQLStore) GetDeposit(ctx context.Context, depositID string) (
	...
	return sqlcDepositToDeposit(
		sqlc.GetAssetDepositsRow(sqlDeposit), &s.addressParams,
	)
}
...
func sqlcDepositToDeposit(sqlDeposit sqlc.GetAssetDepositsRow,
	...
	kit, err := NewKit(
		clientScriptPubKey, clientInteralPubKey, serverPubKey,
		clientKeyDesc.KeyLocator, asset.ID(sqlDeposit.AssetID),
		uint32(sqlDeposit.Expiry), addressParams,
	)
	if err != nil {
		return Deposit{}, err
	}
	...
	if sqlDeposit.Outpoint.Valid {
		...
		deposit.Outpoint = *outpoint
	}

	if sqlDeposit.TimeoutSweepAddr.Valid {
		deposit.TimeoutSweepAddr = sqlDeposit.TimeoutSweepAddr.String
	}
