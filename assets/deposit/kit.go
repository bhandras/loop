...
type Kit struct {
	// FunderScriptKey is a public key owned by the funder that serves two
	// purposes:
	// 1. It acts as the script key for the deposit timeout sweep and as the
	//    key used when constructing the deposit spending zero fee HTLC.
	// 2. It is used as an ephemeral key to derive a shared key with the
	//    node's key. This shared key is then used to derive the funder's
	//    internal deposit key, which can later be revealed.
	FunderScriptKey *btcec.PublicKey

	// FunderInternalKey is generated from the shared key derived from the
	// funder's script key and another key owned by the funder's node. It is
	// used for the derivation of the joint MuSig2 internal key of the
	// deposit.
	...
	// key depending on which side constructs Kit.
...
func NewKit(funderScriptKey, funderInternalKey, coSignerKey *btcec.PublicKey,
...
func (d *Kit) genTimeoutPathSiblingPreimage() ([]byte, *chainhash.Hash, error) {
	...
	preimage, hash, err := commitment.MaybeEncodeTapscriptPreimage(
		siblingPreimage,
	)
	if err != nil {
		return nil, nil, err
...
func (d *Kit) PkScriptFromTransferProof(proofBlob []byte) ([]byte, error) {
	transferProof, err := proof.Decode(proofBlob)
	if err != nil {
		log.Errorf("Unable to decode proof blob: %v", err)
		return nil, nil
	}

	_, siblingHash, err := d.genTimeoutPathSiblingPreimage()
	if err != nil {
		return nil, err
	}
	...
	assetCommitment, err := commitment.FromAssets(
		&version, &transferProof.Asset,
	)
	if err != nil {
		log.Errorf("Unable to create asset commitment: %v", err)
		return nil, nil
	}

	return tapscript.PayToAddrScript(
		*d.muSig2Key.PreTweakedKey, siblingHash,
		*assetCommitment,
	)
}
