...
func (m *Manager) RevealDepositKeys(ctx context.Context,
	depositIDs []string) (map[string]*btcec.PrivateKey, error) {
	...
	keys := make(map[string]*btcec.PrivateKey, len(depositIDs))
	for _, depositID := range depositIDs {
		...
		internalPrivKey, _, err := m.DeriveDepositInternalKey(
